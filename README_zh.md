...
编辑 `user/` 中的文件，写入你自己的音乐偏好、日常习惯和情绪规则。`user/` 目录已被 git 忽略，不会被提交到开源仓库。
...
NeteaseCloudMusicApi 可以在未登录状态下搜索公开歌曲，但很多歌曲需要登录账号或会员权限才能返回可播放链接。

推荐流程：

1. 在浏览器中打开网易云音乐官网并正常登录。
2. 打开浏览器开发者工具。
3. 进入 Network 或 Application / Storage 面板。
4. 找到网易云音乐域名下的 Cookie。
5. 复制包含 `MUSIC_U=...` 的完整 Cookie 字符串。
6. 在 AirFM 中点击 `IMPORT COOKIE`。
7. 粘贴 Cookie 并保存。

AirFM 会把导入的 Cookie 保存在本地：
...
然后编辑复制出来的文件。请不要把个人听歌历史、日常作息和私人歌单细节提交到开源仓库。
